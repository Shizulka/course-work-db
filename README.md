...
Цей бекенд‑застосунок для керування бібліотечною системою. Проєкт реалізує бізнес‑логіку роботи з читачами, книгами, примірниками книг, статусами користувачів та сповіщеннями. Основна мета — продемонструвати проєктування реляційної бази даних, роботу з ORM та створення складних SQL‑запитів.

Робили: Ліщук Ярина та Оситенко Мирослава з ІМ-42.
...
├── alembic.ini                    # Файл конфігурації для міграцій
├── CONTRIBUTIONS.md               # Список учасників і хто що робив
├── docker-compose.yml             # Файл для керування контейнерами
├── Dockerfile                     # Інструкція для створення образу
├── pytest.ini                     # Що б тести починалися тут
...
**Сценарій:4** Повернення книги.Є декілька сценарівї повернення книги.
_Звичайне_ - без пропущеного сроку здачі.
_Загубили_ - тоді в copy_number та віднімається available копія книги.
...
```JSON
{
  "message": "'Залізна Вдова' has been successfully returned.",
  "new_availability": 4
}
```
...
```JSON
{
  "message": "The book 'Pounded In The Butt By My Book \"Pounded In The Butt By My Own Butt\"' was marked as lost. Fine: 126.00 UAH."
}
```
...
`source venv/bin/activate `- активує віртуальне середовище Python. Це щоб бібліотеки проекту не плуталися з системними.

`alembic init alembic` - вона створює папку alembic та файл alembic.ini, де будуть жити налаштування бази даних.

`python -m alembic revision --autogenerate -m "init"` - Alembic порівнює твої Python-моделі (код) з поточною базою даних і створює міграцію про те, які таблиці треба створити або змінити.

`docker-compose up --build` - запускає докер.

`docker-compose down `- зупиняє і видаляє контейнери.

`docker exec -it library_backend bash `- підключається до працюючого контейнера і відкриває там командний рядок (bash), наче це окремий комп'ютер.

`alembic revision --autogenerate -m "initial schema"` - те саме, що пункт 3, але виконується вже всередині контейнера.

`alembic upgrade head` - бере створені файли-інструкції (міграції) і фізично створює/змінює таблиці в базі даних PostgreSQL. head означає «оновити до найсвіжішої версії».

`docker exec -it library_db psql -U postgres -d library` - команда що б зайти в систему і на таблички подивитись.
...
Додали комплексний сценарій створення сутності "Видача книги з черги очікування" , сценарій якщо книгу загубили та сценарій продовження терміну видачі. Та взагалом купа косметичних змін.
...
Добавили можливість видаляти та редагувати книгу та патрона.Почали робити тести. Та ще купа маленьких змін.
...
Закінчили з тестами та селект запитами , по фіксили код тааа добавили докер та міграцію.
